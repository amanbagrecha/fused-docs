...
# Write
df.to_csv("s3://fused-users/basic-tier/<handle>/my-data/output.csv", index=False)
```
...
# Write
df.to_parquet("s3://fused-users/basic-tier/<handle>/my-data/output.parquet")
```
...
# Write
df.to_json("s3://fused-users/basic-tier/<handle>/my-data/output.json")
```
...
# Write
gdf.to_parquet("s3://fused-users/basic-tier/<handle>/my-data/output.parquet")
```
...
# Write
gdf.to_file("s3://fused-users/basic-tier/<handle>/my-data/output.geojson", driver="GeoJSON")
```
...
|----------|--------|---------|
| Fused home directory | `s3://` | `s3://fused-users/basic-tier/<handle>/my-data/file.parquet` |
